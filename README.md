...
- **Pago en efectivo**: desde **Administrador → Configuraciones** se pueden habilitar o deshabilitar los métodos (Mercado Pago, PayPal y efectivo) y definir el mensaje que recibirán los clientes al solicitar pago presencial. Cuando está activo, el checkout del cliente muestra la opción “Pago en efectivo”, genera un comprobante con instrucciones y registra el movimiento en la base para seguimiento administrativo.
...
- **Migraciones requeridas**: `php bin/console doctrine:migrations:migrate` agrega los campos `nickname`/`email` a `credenciales_mercado_pago`, crea la relación entre partners y sus credenciales de Mercado Pago, almacena el `application_fee` de cada pago para el panel del administrador e introduce `cash_payment` junto con los toggles de métodos de pago en `plataforma` (ver `Version20241123093000`).
